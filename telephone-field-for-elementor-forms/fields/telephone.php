...
class Yeekit_Telephone_Field extends \ElementorPro\Modules\Forms\Fields\Field_Base
{
	...
	public function validation($field, $record, $ajax_handler)
	{
		if (! empty($field['required']) && $field['required'] == "yes") {
			if (strlen($field['value']) < 7 || strlen($field['value']) > 16) {
				$ajax_handler->add_error($field['id'], esc_html__('Invalid phone format', 'elementor-pro')); //phpcs:ignore WordPress.WP.I18n.TextDomainMismatch
			}
		}
		if ($field['value'] != "") {
			$datas_submit = map_deep(wp_unslash($_POST['form_fields']), 'sanitize_text_field'); //phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.NonceVerification.Missing
